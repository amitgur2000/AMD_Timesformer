...
def train_epoch(
    ...
    for cur_iter, (inputs, labels, _, meta) in enumerate(train_loader):
        ...
        if cfg.DETECTION.ENABLE:
            preds, _, _, _ = model(inputs, meta["boxes"])    # AG added for explainability
            
        else:
            preds, _, _, _  = model(inputs)     # AG added for explainability
            
        # Compute the loss.
        if cfg.MODEL.NUM_CLASSES == 1:                  # to solve the issue of regression loss
            labels=labels.reshape(preds.shape)   # AG reshape a 1D labels tensor to 2D with second DIM=1 (same as preds)
        modified_labels = torch.zeros_like(preds)
        for i, lab in enumerate(labels):
            modified_labels[i, 0:int(lab)+1] = 1
        if cfg.MODEL.CLASSIFY_TYPE == "ordinal_regress":
            loss = loss_fun(preds, modified_labels)
            print("loss: ", loss)
            print("preds: ", preds)
            print("labels: ", modified_labels)
        else:
            loss = loss_fun(preds, labels)
#        print("loss:", loss.item())   # AG for debug
#        print("pred_0*100:",preds[0].item()*100)
#        print("labels_0*100:",labels[0].item()*100)
        ...
        if cfg.DETECTION.ENABLE:
            ...
            if cfg.DATA.MULTI_LABEL:
                ...
                # Compute the errors.
                top1_err, top5_err, preds_2_labels = metrics.calc_top_1_top_5(cfg, preds, labels)
                # Gather all the predictions across all the devices.
                if cfg.NUM_GPUS > 1:
...
def eval_epoch(val_loader, model, val_meter, cur_epoch, cfg, writer=None):
    ...
    for cur_iter, (inputs, labels, _, meta) in enumerate(val_loader):
        ...
        if cfg.DETECTION.ENABLE:
            ...
            preds = model(inputs)
            #### AG added to see loss also in val state
            #loss_fun = losses.get_loss_func(cfg.MODEL.LOSS_FUNC)(reduction="mean")
            if cfg.MODEL.NUM_CLASSES == 1:                  # to solve the issue of regression loss
                labels=labels.reshape(preds.shape)   # AG reshape a 1D labels tensor to 2D with second DIM=1 (same as preds)
            #loss = loss_fun(preds, labels)
            if cfg.DATA.MULTI_LABEL:
                ...
                top1_err, top5_err, preds_2_labels = metrics.calc_top_1_top_5(cfg, preds, labels)
                if cfg.NUM_GPUS > 1:
                   # top1_err, top5_err, loss = du.all_reduce([top1_err, top5_err, loss])   #AG added loss
                    top1_err, top5_err = du.all_reduce([top1_err, top5_err])   #AG added loss

                # Copy the errors from GPU to CPU (sync point).
               # top1_err, top5_err, loss = top1_err.item(), top5_err.item(), loss.item()   # AG added loss
                top1_err, top5_err = top1_err.item(), top5_err.item()  
               # print("val loss:", loss)  # Added loss print
                val_meter.iter_toc()
                # Update and log stats.

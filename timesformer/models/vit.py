...
class Attention(nn.Module):
    ...
    def forward(self, x, head_mask=None):   # AG added for explainability
        B, N, C = x.shape
        if self.with_qkv:
        ...
        if head_mask is not None:
            attn = attn * head_mask.view((1, head_mask.shape[0], 1, 1))     # AG added for explainability
...
class Block(nn.Module):
    ...
    def forward(self, x, B, T, W, head_mask):
        ...
        attn_temporal, attn_spatial = None, None         # AG added for explainability
        if self.attention_type in ['space_only', 'joint_space_time']:
            x = x + self.drop_path(self.attn(self.norm1(x)))
            x = x + self.drop_path(self.mlp(self.norm2(x)))
            return x, attn_temporal, attn_spatial   # AG added for explainability
        elif self.attention_type == 'divided_space_time':
            ## Temporal
            xt = x[:,1:,:]
            xt = rearrange(xt, 'b (h w t) m -> (b h w) t m',b=B,h=H,w=W,t=T)
            norm_xt = self.temporal_norm1(xt)      # AG added for explainability
            ...
            norm_xs = self.norm1(xs)     # AG added for explaniability
...
class VisionTransformer(nn.Module):
    ...
    def forward_features(self, x, output_attentions, head_mask):
        ...
        ## Attention blocks
        all_attn_temporal = []     # AG added for explainability (block of code)
        all_attn_spatial = []
        all_hidden_states = []
    ...
    # AG added for explainability. Changed all function
...
# AG fixed all API to support same API as vit_base_patch16_224
#    def __init__(self, img_size=224, patch_size=16, num_classes=400, num_frames=8, attention_type='divided_space_time',  pretrained_model='', **kwargs):
    ...
    # AG for explainability, changed all function

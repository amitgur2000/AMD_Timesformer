...
class TestMeter(object):
    ...
    # AG added support for top-2/3/4:
    def finalize_metrics(self, cfg, ks=(1, 5)):
        """
        Calculate and log the final ensembled metrics.
        ks (tuple): list of top-k values for topk_accuracies. For example,
            ks = (1, 5) correspods to top-1 and top-5 accuracy.
        """
        # AG: TODO need to debug the warning
        ...
        if self.multi_label:
            ...
            topks=[0,0]
            topks[0], topks[1], preds_2_labels = metrics.calc_top_1_top_5(cfg, self.video_preds, self.video_labels)
            ...
            # ROC OvR for all classes
            num_classes_m1 = num_classes-1
            all_classes_roc_auc=roc_auc_score(y_true, pred_scores_sm, multi_class='ovr')



            print("AUC all classes, OvR: ", all_classes_roc_auc)
            ...
            # One vs Rest for last categoty (in case of AMD: wet vs. non-wet)
            for OvR_class in range(0,num_classes):
                y_true_OvR = y_true.copy()      
                y_true_OvR[y_true_OvR==OvR_class]=-1
                y_true_OvR[y_true_OvR!=-1]=0
                y_true_OvR[y_true_OvR==-1]=1
                
                # for binary of OvR, take the maximum of rest
                pred_scores_others = np.max(np.delete(pred_scores, OvR_class, axis=1),axis=1)
                pred_scores_OvR = np.vstack((pred_scores_others, pred_scores[:,OvR_class])).T
                pred_scores_sm_OvR = np.array([softmax(p_s) for p_s in pred_scores_OvR])
                fpr[OvR_class], tpr[OvR_class], thresholds_last = roc_curve(y_true_OvR, pred_scores_sm_OvR[:,1])
                roc_aus[OvR_class]=roc_auc_score(y_true_OvR, pred_scores_sm_OvR[:,1])
                print("AUC on Class ",OvR_class,"=", roc_aus[OvR_class])
                pred_labels[OvR_class] = [np.argmax(p_s) for p_s in pred_scores_OvR] 
                conf_matrix = confusion_matrix(y_true_OvR, pred_labels[OvR_class], labels=(0,1))
                print("Conf matrix OvR for class ", OvR_class, "= \n", conf_matrix, "\n")
  
  
            # One vs rest for first categoty (in case of AMD: healthy vs. others)
     
            # Plot ROCs:
            plt.figure()
            colors = ['aqua', 'darkorange', 'cornflowerblue', 'darkblue', "deeppink"]
            for i,color_i in zip(range(0,num_classes),colors):
                plt.plot(fpr[i], tpr[i], color=color_i, lw=2, label=str(i)+': ROC curve OvR (area = %0.3f)' % roc_aus[i])
            # plt.plot(fpr_last, tpr_last, color='darkblue', lw=2, label='ROC curve OvR on Wet (area = %0.3f)' % last_OvR_roc_auc)
            ...
            plt.savefig('OvR_roc_curve.jpg')



            # print("y_true:",y_true)
            if cfg.MODEL.CLASSIFY_TYPE == "ordinal_regress":
                pred_labels= preds_2_labels
                pred_labels_last=0
                pred_labels_first=0
            else:
                pred_labels = [np.argmax(pred_score) for pred_score in pred_scores]
                cls_report = classification_report(y_true, pred_labels, labels=range(0,num_classes))
            ...
            # print("Prediction, top_1 and top_2 %:", pred_scores_sm[:,0:1])
            # print("Confidence:", pred_scores_sm)
            np.set_printoptions(suppress=True)
            np.set_printoptions(precision=2)
            new_labels=list(range(0, self.num_cls))
            conf_matrix = confusion_matrix(y_true, pred_labels, labels=new_labels)

            print("Confusion Matrix all classes: \n", conf_matrix)

            ### AG for the case of 2 classes for wet and other classes for non-wet:
            if num_classes>7:
                wet_offset=2
            else:
                wet_offset=1
            acc_class_2=(sum(sum(conf_matrix[0:num_classes-wet_offset,0:num_classes-wet_offset]))+sum(sum(conf_matrix[num_classes-wet_offset:num_classes,num_classes-wet_offset:num_classes])))/sum(sum(conf_matrix))
            print("Accuracy of wet vs. non-wet: %.2f%%" % (acc_class_2 * 100.0))
            video_files=[]
            ...
            save_err_dir="/home/agur/projects/AMD/AMD_Timesformer/tmp_videos/"
            # if os.path.exists(save_err_dir):
            #     shutil.rmtree(save_err_dir)
            # os.mkdir(save_err_dir)
            for img, (ys, pred, pred_conf, video_file) in enumerate(zip(y_true, pred_labels,pred_scores_sm, video_files)):
                significant_error = (abs(ys-pred)>1) and pred_conf[ys]<(0.5*1/num_classes)
                if ys != pred and significant_error:
                    file_name_list = video_file[0].split("/")
                    file_name= file_name_list[-2]+"_"+file_name_list[-1]
                    #print("error, video #, y_true, predict, conf:", img, ys, pred, pred_conf)
                    print(ys, pred, pred_conf, video_file[0], file_name)
                    cmd = 'cp '+video_file[0]+" "+save_err_dir+file_name
                    os.system(cmd)            

            # self.stats["AUC"] = roc_auc_score(y_true, pred_scores_sm[:, 1]) 
...
class TrainMeter(object):
    ...
    def update_stats(self, top1_err, top5_err, loss, lr, mb_size, stats={}):
        ...
        self.lr = lr
        self.loss_total += loss  # * mb_size  # AG, don't need to multiply the loss by mini-batch. Using loss mean, not sum

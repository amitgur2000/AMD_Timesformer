...
import numpy as np
import os #HEZI
import timesformer.utils.metrics as metrics   # AG added fpr calc_top1_top5
...
# AG: added a function to calculate topk in cases of less than 5 classes, in case of regression and in case or ordinal classification 
def calc_top_1_top_5(cfg, preds, labels):
    TH = 0
    num_topk = cfg.MODEL.NUM_CLASSES if cfg.MODEL.NUM_CLASSES<5 else 5
    if cfg.MODEL.CLASSIFY_TYPE == "ordinal_regress":
        preds_2_label = (preds > TH).cumprod(axis=1).sum(axis=1) - 1
        num_topks_correct = [(preds_2_label==labels).sum(),1]
        num_topk=1
    else:
        num_topks_correct = metrics.topks_correct(preds, labels, (1, num_topk))  # Ag TO SUPPORT ONLY 2 labels
        preds_2_label = torch.argmax(preds,dim=1)
    if num_topk==5:
        top1_err, top5_err = [
        ...
        top5_err = top1_err
    return top1_err, top5_err, preds_2_label
